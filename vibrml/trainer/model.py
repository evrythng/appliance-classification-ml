...
from keras.layers import Dropout, Input, Conv1D, MaxPool1D, GlobalMaxPooling1D, Dense, Flatten
...
def generator_input(input_file, batch_size=128):
    """Generator function to produce features and labels
       needed by keras fit_generator.
    """

    while True:
        x_data = np.array(ujson.load(tf.gfile.Open(input_file[0], 'rb')))
        y_data = np.array(ujson.load(tf.gfile.Open(input_file[1], 'rb')))


        for i in range(0, len(x_data), batch_size):
            x = x_data[i:min(i+batch_size, len(x_data))]
            y = y_data[i:min(i+batch_size, len(y_data))]
            yield x, y
