  ...
  def on_epoch_begin(self, epoch, logs={}):
    if epoch > 0 and epoch % self.eval_frequency == 0:
      ...
      if len(checkpoints) > 0:
        checkpoints.sort()
        chair_model = load_model(checkpoints[-1])
        chair_model = model.compile_model(chair_model)
        loss, acc = chair_model.evaluate_generator(
            model.generator_input(self.eval_files),
            steps=self.steps)
        print('\nEvaluation epoch[{}] metrics[{:.2f}, {:.2f}] {}'.format(
            epoch, loss, acc, chair_model.metrics_names))
...
def dispatch(train_files,
  ...
  if job_dir.startswith("gs://"):
    chair_model.save(CHAIR_MODEL)
    copy_file_to_gcs(job_dir, CHAIR_MODEL)
  else:
    chair_model.save(os.path.join(job_dir, CHAIR_MODEL))

  # Convert the Keras model to TensorFlow SavedModel
  model.to_savedmodel(chair_model, os.path.join(job_dir, 'export'))
