...
const {vibrationProperty, transformFnDecorator, standardization} = require('./preprocess.js');
...
function onThngPropertiesChanged(event) {
    ...
    return auth.getAccessToken().then(accessToken => {
        request
            .post("https://ml.googleapis.com/v1/projects/"+modelConfigParams.google_project +"/models/" + modelConfigParams.model_name + "/versions/demobooth_v4:predict")
            .send(inputs)
